...
We will configure the "public" host as the VPN server. (A VPN server must have a "public-facing" interface on the external network, to accept connections from outside, as well as an interface on the internal network.)
...
We also need to make sure the OpenVPN instance will forward traffic, so on "public" run:
...
so that the traffic destined for the VPN tunnel will go to the "public" node.
...
* IP address of the VPN server, "public" (i.e. the fact that the client is using this particular VPN server)
