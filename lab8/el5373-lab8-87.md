...
**Note**: Students have reported problems using Microsoft Edge for this exercise - some Edge browser versions don't signal keepalive support in the HTTP header. I recommend using a Chrome, Firefox, or Safari browser instead. If you see the same number of TCP connections with and without keepalive, that's OK - just make sure you show that the *server* headers show that it does not support keepalive, in the scenario where you turn off keepalive. (Some browsers open multiple parallel TCP sessions automatically at the beginning of an HTTP session, so that it can use them immediately without having to wait for the TCP connection establishment. If your browser does this, you'll see multiple TCP connections even with KeepAlive on.)
