...
for i in range(4):
    ...
    router_node.execute("curl -s https://deb.frrouting.org/frr/keys.asc | sudo apt-key add -", quiet=True)
    router_node.execute("echo deb https://deb.frrouting.org/frr $(lsb_release -s -c) frr-stable | sudo tee -a /etc/apt/sources.list.d/frr.list", quiet=True)
